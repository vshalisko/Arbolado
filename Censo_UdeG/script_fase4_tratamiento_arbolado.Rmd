...
output_basename <- 'tabla_fase_4_'

# Superficies de los centros (CUNORTE,CUVALLES)
superficies <- c(26.65,24.83)
...
n_por_especie_resto <- n_por_especie_ordered[1:(nrow(n_por_especie_ordered)-20),]
n_por_especie_resto_linea <- data.frame(Especies="Resto de las especies",N=sum(n_por_especie_resto$N))
n_por_especie_ordered <- n_por_especie_ordered[(nrow(n_por_especie_ordered)-20):nrow(n_por_especie_ordered),]
n_por_especie_ordered <- rbind(n_por_especie_resto_linea,n_por_especie_ordered)
...
ab_por_especie_resto <- ab_por_especie_ordered[1:(nrow(ab_por_especie_ordered)-20),]
ab_por_especie_resto_linea <- data.frame(Especies="Resto de las especies",AB=sum(ab_por_especie_resto$AB))
ab_por_especie_ordered <- ab_por_especie_ordered[(nrow(ab_por_especie_ordered)-20):nrow(ab_por_especie_ordered),]
ab_por_especie_ordered <- rbind(ab_por_especie_resto_linea,ab_por_especie_ordered)
...
for (i in 1:length(list_centers) ) {
  ...
  abline(v = 0, lty = 1, lwd = 1)  
  abline(v = seq(from = 50, to = 1.05 * max(n_por_especie_centro_solo_ordered$N), by = 50),
               lty = 2, lwd = 1, col = "darkgray") 
}
```
